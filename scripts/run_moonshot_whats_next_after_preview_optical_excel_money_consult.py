"""Moonshot — What's next with SoftDent Excel grey + Preview optical-only (CONSULT ONLY).
...
HARD TRUTH (operator — do not contradict):
- SoftDent Excel greyed = SoftDent INTENTIONALLY blocking extractable pulls across ALL Output Options reports.
- Print Preview is UNIVERSAL optical fallback — NOT a substitute money-beam path. Do NOT recommend
  "always use Print Preview" for money ingest / OCR / inventing dollars (that fights SoftDent's pull block).
- Excel when SoftDent allows it → C:\\SoftDentReportExports → NR2 ingest. Excel grey → Preview optical only.
- Live re-probe 2026-07-17 ~06:00: Excel STILL grey on Aging + Collections. morning_bundle refresh BLOCKED.
  Prior morningBundle.ok=true (aging+register from when Excel was clickable) remains money-beam truth.
- SoftDent Excel enablement is ATTENDED OPS (Carestream/operator) — Cursor cannot code SoftDent un-grey.
  Do NOT pick SoftDent Excel enablement as #1 Cursor-apply package again unless framing it as WAIT/ATTENDED only.

ALREADY APPLIED: viewport P1–4+Hub; morning Track B; collections grey harden; claims honesty; desk smoke;
pilot Day X of 30; SoftDent grey-Excel universal docs (through 7bebd5b).

OTHER: Trellis withBenefits WAIT ~2026-07-20. ERA README placeholder only (empty ≠ $0). forceClose false.
UI polish CLOSED.

YOUR JOB: Pick THE single best NEXT that Cursor can APPLY NOW in code/docs/ops wiring OR an honest
WAIT / ATTENDED OPS if nothing else is implementable. Prefer product value from LIVE AUDIT.

CANDIDATES (pick ONE #1):
1) WAIT / ATTENDED — SoftDent Excel Carestream enablement (document hold; no SoftDent GUI spam)
2) ERA real .835 ingest — only if LIVE AUDIT shows a real remittance file (not README)
3) WAIT — Trellis withBenefits until ~2026-07-20
4) Something else from LIVE AUDIT only — real NewRidgeFinancial2/ paths (no SoftDent Excel GUI while grey;
   no Preview-as-money; no forceClose flip; no React/chat embeds)

HARD CONSTRAINTS:
- SoftDent READ-ONLY; empty ≠ $0; PHI initials+hash
- Never Printer/File; never invent Preview dollars; never invent withBenefits/835
...
def live_audit() -> dict:
    ...
    return {
        "operatorPreviewVsExcel": {
            "previewUniversalOptical": True,
            "previewNotMoneyBeam": True,
            "excelWhenClickableForMoney": True,
            "excelStillGreyLive": True,
        },
        ...
        "trellisWaitUntil": "~2026-07-20 1AM/2AM",
        "periodClose": {
        ...
        "pilot": pilot,
        "lastCommits": ["7bebd5b SoftDent Excel still grey + attended-ops consult"],
    }
...
def chat(system: str, user: str, api_key: str, base_url: str) -> tuple[str, dict]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "X-Title": "NR2 Moonshot next Preview optical Excel money",
        },
    )
...
def main() -> int:
    ...
    (OUT / f"moonshot_whats_next_preview_optical_excel_money_audit_{stamp}.json").write_text(
        json.dumps(audit, indent=2), encoding="utf-8"
    )
    user = (
        "LIVE AUDIT. SoftDent Excel grey; Preview optical only (not money). "
        "Pick Cursor-apply NEXT or honest WAIT/ATTENDED.\n\n"
    ...
    if not content.strip():
        ...
        return 1
    report = DOCS / f"MOONSHOT_WHATS_NEXT_AFTER_PREVIEW_OPTICAL_EXCEL_MONEY_{DATE}.md"
    header = (
        f"# Moonshot AI — What's Next (Preview Optical / Excel Money) (CONSULT ONLY)\n\n"
        f"**Date:** {DATE}\n"
        f"**Model:** `{meta.get('model')}`\n"
        f"**Key:** {key_name}\n"
        f"**Script:** `scripts/run_moonshot_whats_next_after_preview_optical_excel_money_consult.py`\n"
