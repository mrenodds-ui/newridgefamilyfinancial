"""Moonshot — What's next after pilot shadow clock hygiene (CONSULT ONLY).
...
- SoftDent agent hard-stop LIFTED. SoftDent GUI allowed under read-only Excel|Preview rules.
- JUST APPLIED on main (91a7b9e): Pilot shadow clock hygiene — optical SHADOW gate shows
  Day X of 30 · N left (UTC calendar days). forceCloseAvailable stays false.
  Doc: MOONSHOT_PILOT_SHADOW_CLOCK_HYGIENE_APPLIED_2026-07-17.md
- Also APPLIED recently: viewport P1–4 + Hub; morning_bundle Track B (ok=true, failed=[collections]);
  collections Excel-greyed harden (77dbc45); claims honesty; HAL desk smoke; ERA first-drop path
  (README placeholder only — empty ≠ $0); pilot shadow clock start.
- SoftDent Output Options: Excel radio GREYED live (aging + collections probed). Operator SoftDent
  enablement required (runbook softdent_excel_enablement_nr2.md). Do not spam SoftDent GUI
  until Excel is clickable (print risk). Preview-only is honest but not money-beam ingest.
- morningBundle.ok=true with prior aging+register Excel paths; money beams LIVE.
- Trellis AM: awaiting; withBenefits wait until ~2026-07-20 1AM/2AM scrape. Do NOT invent.
- UI polish P1–P10 / V1–V6 / E1–E4 CLOSED — no reskin.
- Shadow clock visibility is DONE — do not redo.

YOUR JOB: Pick THE single best NEXT package. Prefer implementable product/ops work from LIVE AUDIT.
If the only honest next is WAIT or ATTENDED OPS (operator SoftDent Excel / payer 835), say so clearly.

CANDIDATES (pick ONE #1):
1) SoftDent Excel enablement — ATTENDED OPS (operator Carestream/IT make Excel clickable) then
   morning_bundle_attended --yes --refresh-close. Code-only only if LIVE AUDIT shows a real NR2 gap.
2) ERA real first remittance drop — ingest real .835 (not README). Blocked on payer file if none.
3) WAIT — Trellis withBenefits until ~2026-07-20 (only if truly highest-value honest next).
4) Something else justified ONLY from LIVE AUDIT — real NewRidgeFinancial2/ paths.
...
- Do NOT invent ClearCoverage withBenefits or SoftDent Excel dollars from Preview
- Prefer not to SoftDent GUI until Excel clickable
...
def live_audit() -> dict:
    ...
    return {
        "justApplied": "pilot shadow clock hygiene 91a7b9e — Day X of 30 on optical desk",
        "softdentExcelGreyedLive": True,
        "excelEnablementRunbook": "NewRidgeFinancial2/docs/runbooks/softdent_excel_enablement_nr2.md",
        "serverUp": True,
        "trellisTasks": {
            "nightly1am": "next ~2026-07-20 1:00 Mon-Thu",
            "proof2am": "next ~2026-07-20 2:00 Mon-Thu",
        },
        "trellisAmProof": {
            "ok": am.get("ok"),
        ...
        "appliedAlready": [
            ...
            "collections excel-greyed 77dbc45",
            "claims honesty",
            "HAL desk smoke",
            "pilot shadow start + Day X of 30 hygiene 91a7b9e",
            "ERA path wiring (placeholder README)",
        ],
    }
...
def chat(system: str, user: str, api_key: str, base_url: str) -> tuple[str, dict]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "X-Title": "NR2 Moonshot next after shadow clock hygiene",
        },
    )
...
def main() -> int:
    ...
    (OUT / f"moonshot_whats_next_after_shadow_hygiene_audit_{stamp}.json").write_text(
        json.dumps(audit, indent=2), encoding="utf-8"
    )
    user = (
        "LIVE AUDIT follows. Shadow Day X of 30 hygiene DONE. SoftDent Excel greyed. "
        "Trellis wait ~2026-07-20. Pick implementable NEXT now.\n\n"
    ...
    if not content.strip():
        ...
        return 1
    report = DOCS / f"MOONSHOT_WHATS_NEXT_AFTER_SHADOW_CLOCK_HYGIENE_{DATE}.md"
    header = (
        f"# Moonshot AI — What's Next After Shadow Clock Hygiene (CONSULT ONLY)\n\n"
        f"**Date:** {DATE}\n"
        f"**Model:** `{meta.get('model')}`\n"
        f"**Key:** {key_name}\n"
        f"**Script:** `scripts/run_moonshot_whats_next_after_shadow_clock_hygiene_consult.py`\n"
