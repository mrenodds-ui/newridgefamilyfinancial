"""Moonshot AI — What's next after SoftDent GUI export hardening (CONSULT ONLY).

Operator: next
Just shipped: 1e13fd0 / db112a5 SoftDent export retries + attest-only fallback;
also 45ea727 landing money-beams; 23f68df period-close on hub/OM.
"""
...
JUST SHIPPED on main (through db112a5 → nr2/main), build around nr2-12033-softdent-export-harden:

- 1e13fd0 / db112a5 — SoftDent GUI Excel export retries + min-bytes validation;
  period-close falls back to attest-only on GUI failure (no stall)
- 45ea727 — Cite money-beams on main landing; demote LIVE when close stalls
- 23f68df — Surface period-close status on Pages Hub and Office Manager
- cde7ef0 — Morning SoftDent aging auto-pull
- Prior today: optical SoftDent/QB beam bind, consent-free SoftDent export,
  SoftDentReportExports path hygiene, money honesty, BlueNote-only announce

PRIOR APPLIED runner-ups still open (from SoftDent harden package):
1) HAL Force Close optical control (OM / Pages Hub — laser-gated, JSONL attest)
2) Formal beamHash desk proof across HAL chat + optical pages
...
YOUR JOB:
Pick THE single best NEXT package now that SoftDent export hardening is on main.
Prefer one clear next. Prefer OPS if data truth / desk ops blocked; CODE when UX/honesty gap.
Do NOT redo: SoftDent export retries/attest-only fallback, morning SoftDent auto-pull,
money-beam optical/landing bind, period-close hub surface, consent-free SoftDent export,
path hygiene, money honesty, BlueNote chrome filter.

CANDIDATES (pick ONE as THE next):
1) HAL Force Close optical control (OM / Pages Hub — laser-gated, JSONL attest)
2) Formal beamHash desk proof (HAL chat + optical AR/QB cite identical hash on live 8765)
3) BlueNote alert when period-close uses attest_only / laser-blocks / stalls
4) Expand morning SoftDent pull to aging+register+collections (hardened path)
5) Wire QB sync consent UX polish or schedule (SoftDent pull is free; QB still gated)
6) Desk smoke script: close status + money-beams + optical provenance identity check
...
def live_audit() -> dict:
    ...
    return {
        ...
        "build": build,
        "commitHint": "1e13fd0 SoftDent export harden · 45ea727 landing beams · 23f68df hub period-close",
        "lastCloseLog": last_close,
        "forceCloseExists": False,  # not shipped; operator-facing Force Close button
        "live": {
...
def main() -> int:
    ...
    excerpts = []
    for name, lim in (
        ("MOONSHOT_WHATS_NEXT_AFTER_SOFTDENT_PULL_APPLIED_2026-07-15.md", 3500),
        ("MOONSHOT_WHATS_NEXT_AFTER_SOFTDENT_PULL_2026-07-15.md", 2500),
        ("MOONSHOT_WHATS_NEXT_AFTER_OPTICAL_BEAM_BIND_APPLIED_2026-07-15.md", 2000),
    ):
        path = DOCS / name
        if path.is_file():
            excerpts.append(
    ...
    if "openrouter" in url.lower():
        ...
        headers["X-Title"] = "NR2 Whats Next After SoftDent Harden"
    ...
    (OUT / f"moonshot_whats_next_after_softdent_harden_{stamp}.json").write_text(
        json.dumps(raw, indent=2)[:400000], encoding="utf-8"
    )
    (OUT / f"moonshot_whats_next_after_softdent_harden_audit_{stamp}.json").write_text(
        json.dumps(audit, indent=2, default=str)[:200000], encoding="utf-8"
    )

    header = (
        f"# Moonshot AI — What's Next After SoftDent Export Hardening (CONSULT ONLY)\n\n"
        f"**Date:** {DATE}\n"
        f"**Model:** `{model}`\n"
        f"**Key:** {key_name}\n"
        f"**Status:** {status}\n"
        f"**Repo root:** `{REPO}`\n"
        f"**Prior:** `1e13fd0` SoftDent export harden · `45ea727` landing beams\n"
        f"**Script:** `scripts/run_moonshot_whats_next_after_softdent_harden_consult.py`\n"
        f"**Apply:** DO NOT APPLY until operator approves.\n\n"
        f"## Operator request (verbatim)\n\n> {OPERATOR_REQUEST_VERBATIM}\n\n---\n\n"
    )
    md = header + (text.strip() or "_(empty)_") + "\n"
    md_path = DOCS / f"MOONSHOT_WHATS_NEXT_AFTER_SOFTDENT_HARDEN_{DATE}.md"
