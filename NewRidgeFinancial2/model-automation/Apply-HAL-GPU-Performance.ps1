<#
.SYNOPSIS
  Apply Ollama GPU performance settings for single 32B on R9700 32 GB.

.DESCRIPTION
  Sets Ollama env for one loaded model on the discrete AMD GPU, creates
  hal-local:32b (qwen3:32b Q4_K_M), pins only that model, and verifies GPU load.
...
$installScript = Join-Path $scriptRoot "Install-HAL-GPU-Single-32B.ps1"
...
Write-Host "`n=== HAL GPU performance setup (single 32B · R9700 32 GB) ===" -ForegroundColor Cyan
...
if (-not $SkipPin) {
    Write-Host "`nPinning single GPU model (hal-local:32b qwen3:32b Q4_K_M)..." -ForegroundColor Yellow
    & $installScript
    if ($LASTEXITCODE -ne 0) { throw "Install-HAL-GPU-Single-32B failed" }
...
Write-Host "Rollback to 24B: .\Install-HAL-GPU-Single-24B.ps1 then restore git configs if needed." -ForegroundColor DarkGray
