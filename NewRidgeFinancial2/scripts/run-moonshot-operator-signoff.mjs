...
function writeReport() {
  ...
  const moonshotVerdict =
    checklistFails.length === 0 && checklist.filter((r) => r.status === "PASS").length >= 8
      ? "APPROVE hal-10085 — Moonshot Tier S3 semantic zoom, HAL presence, hero mirror, citations (automated sign-off; operator name still required)"
...
async function main() {
  ...
  record(
    ...
    moonshotExtentSrc.includes("hal-10085") && moonshotExtentSrc.includes("Practical ceiling") ? "PASS" : "FAIL",
