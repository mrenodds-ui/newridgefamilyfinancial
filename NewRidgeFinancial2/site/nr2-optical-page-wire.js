...
(function (global) {
  ...
  /** Moonshot E1–E4 + Package 2: page letterhead owns banner; avoid duplicate E1 inject. */
  function bootExecutiveChrome() {
    document.body.classList.add("exec-desk-surface");
    const pageLh = document.getElementById("nr2-page-letterhead");
    const banner = document.querySelector(".banner");
    // Package 2: `#nr2-page-letterhead` already maps title/meta/status — do not inject a second bar.
    if (!pageLh && banner && !document.getElementById("nr2-exec-letterhead")) {
