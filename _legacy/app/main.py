...
def _frontend_asset_headers(asset_path: Path) -> dict[str, str]:
    # The HTML shell must never be reused from a stale browser/HTTP cache.
...
def serve_frontend_app(request_path: str = ""):
    ...
    if asset_path is not None:
        return FileResponse(asset_path, headers=_frontend_asset_headers(asset_path))

    if "." in Path(request_path).name:
        raise HTTPException(status_code=404, detail="Frontend asset not found")

    index_path = FRONTEND_DIST_DIR / "index.html"
    return FileResponse(index_path, headers=_frontend_asset_headers(index_path))
